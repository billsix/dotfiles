...
### Python dependency-install timing — deps at build, editable self-install at runtime (so my host edits propagate)

**The point (Bill, 2026-08-16): when I edit code on the host, outside the container, those edits must
show up *inside* the container with no image rebuild.** The build-vs-runtime split below is the
mechanism that guarantees it, and it is the **default pattern** for every containerized Python project
— deviate only with a reason. It is a *suggested default*, not a hard gate; I don't want a tool, I want
**you to remind me when a project breaks it** (see the reminder note below).

- **Third-party dependencies → installed at IMAGE-BUILD time** in the `Dockerfile`, flag-gated
  (`BUILD_DOCS`/`USE_JUPYTER`/…). They don't change between runs, so baking them means a `make shell` /
  `make docs` fetches nothing. (gacalc: `numpy sympy pyright`, the Sphinx set, `.[dev,notebooks,jupyter]`;
  mvm: `.[dev,notebooks,jupyter]`; hanoi: `-r requirements.txt`.)
- **The project's OWN package → editable self-install at RUN time** in the entrypoint, against the
  **bind-mounted** source: `uv pip install --no-deps --no-index --no-build-isolation -e .`. `--no-index
  --no-deps` = offline (no PyPI, no dependency resolution); `-e .` = imports resolve to the **live
  mounted tree**. **This is the line that propagates my host edits** — change a `.py` on the host, the
  next `import` in the container sees it. (gacalc also regenerates its gitignored `g1/g2/g3` at runtime
  for the same reason.)
- **Do NOT move the runtime `-e .` to build time.** A build-time *non-editable* install freezes a
  snapshot into `site-packages`, so host edits are ignored — the opposite of the goal.

**Reminder note — flag it, don't silently accept it.** When working on a containerized Python project,
tell me if the project: (a) installs third-party deps in an entrypoint instead of the `Dockerfile`;
(b) installs its own package **non-editably** at runtime, or skips the runtime `-e .`, so host edits
won't propagate; or (c) runs a **networked** pip in any entrypoint (a `pip install` *without*
`--no-index`, or one that pulls dependencies) — meaning a run can hit PyPI or a dep was wrongly
deferred. If I have a reason to deviate, fine — just make sure I've seen the trade-off first.
...
`Dockerfile` + `Makefile` + `entrypoint/shell.sh` present? Fedora-44 base with the dnf-cache idiom? `CONTAINER_NAME` matches the dir? `FILES_TO_MOUNT` mounts the repo at `/<name>/:Z`? `help` target with `##`-documented targets? Build-arg defaults `1` (Makefile) / `0` (Dockerfile)? Entrypoint scripts and the image's `ENTRYPOINT`/`--entrypoint` story consistent? Each `entrypoint/*.sh` references the *right* project's paths (a frequent copy-paste drift) — flag any that point at another project? Package installation extracted into host-runnable `entrypoint/0N-install-*.sh` group scripts the Dockerfile dispatches by ARG (per "Host-agnostic setup belongs in a script"), or still inline in a `RUN`? **Python: third-party deps installed at build (Dockerfile) and the project self-installs editable + offline (`-e . --no-index --no-deps`) at runtime so host edits propagate — flag any project that defers deps to an entrypoint or runs a networked runtime pip (per "Python dependency-install timing")?**
